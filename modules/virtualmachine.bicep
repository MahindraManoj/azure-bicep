...
/* vmNamePrefix: ''                //string. virtual machine name prefix. Should not be more than 12 chars
    ...
    vmPrimaryNicSubnet: ''         //string. Subnet where the primary nic of the vm(s) will be deployed
    vmSecondaryNicSubnet: ''       //string or empty String. Subnet where the sec nic of the vm(s) will be deployed (OPTIONAL)
...
resource existingBootDiagSa 'Microsoft.Storage/storageAccounts@2021-08-01' existing = if ((!(vmValues.bootDiagnostics.newStorageAccount) || empty(vmValues.bootDiagnostics.newStorageAccount)) && (vmValues.bootDiagnostics.enabled) && !empty(vmValues.bootDiagnostics.storageAccountRG)) {
  name: vmValues.bootDiagnostics.storageAccountName
  scope: resourceGroup(vmValues.bootDiagnostics.storageAccountRG)
}

// create storage account to configure boot diagnostics based on the user's input
module newBootDiagsa 'storageaccount.bicep' = if (vmValues.bootDiagnostics.enabled && (vmValues.bootDiagnostics.newStorageAccount || empty(vmValues.bootDiagnostics.newStorageAccount))) {
...
module pNicModule 'networkInterface.bicep' = [for vnic in range(1, vmValues.vmCount): {
  name: 'vNicPrimary${vnic}'
  params: {
    vNicVnetName: vmValues.vmVnetName
    vNicPrimarySubnet: vmValues.vmPrimaryNicSubnet
    vnetRG: vmValues.vmVnetRG
    vNicSecondarySubnet: vmValues.vmSecondaryNicSubnet
...
// Conditionally create secondary vNics for the vm(s)
module sNicModule 'networkInterface.bicep' = [for vnic in range(1, vmValues.vmCount): if(!empty(vmValues.vmSecondaryNicSubnet)) {
...
//Create Windows VM(s)
resource windowsvm 'Microsoft.Compute/virtualMachines@2021-11-01' = [for i in range(1, vmValues.vmCount) : {
  name: '${vmValues.vmNamePrefix}-vm${i}'
  location: resourceLocation
  identity: (vmValues.enableVmIdentity) ? {
    type: 'systemAssigned'
  } : json('null')
  properties: {
    availabilitySet: {
      id: !empty(availabilitySetName) ? availset.outputs.Id : json('null')
    }
    hardwareProfile: {
      vmSize: vmSize
    }
    billingProfile: {}
    osProfile: {
      computerName: '${vmValues.vmNamePrefix}-vm${i}'
    ...
    storageProfile: {
      ...
      osDisk: {
        ...
        name: '${vmValues.vmNamePrefix}-vm${i}_OSDisk'
        caching: 'ReadWrite'
        diskSizeGB: vmValues.osDiskSizeGB
        managedDisk: {
          storageAccountType: vmValues.osDiskType
        }
      }
      dataDisks: [for d in range(0, vmValues.vmCount): {
        name: '${vmValues.vmNamePrefix}-vm${i}_DataDisk_${d}'
        diskSizeGB: vmValues.dataDiskSizeGB
        lun: d
        createOption: 'Empty'
        caching: 'None'
        managedDisk: {
          storageAccountType: vmValues.dataDiskType
        }
      }]
    }
    networkProfile: !empty(vmValues.vmSecondaryNicSubnet) ? {
      networkInterfaces:[
        {
          id: pNicModule[i].outputs.vNicResourceId
          properties: {
            primary: true
          }
        }
        {
          id: sNicModule[i].outputs.vNicResourceId
          properties: {
            primary: false
          }
        }
    ...
    licenseType: vmValues.applyAzureHybridBenefit ? 'Windows_Server' : string(null)
    diagnosticsProfile: {
      bootDiagnostics: {
        enabled: vmValues.bootDiagnostics.enabled
        storageUri:  ((!(vmValues.bootDiagnostics.newStorageAccount) || empty(vmValues.bootDiagnostics.newStorageAccount)) && (vmValues.bootDiagnostics.enabled) && !empty(vmValues.bootDiagnostics.storageAccountRG)) ? existingBootDiagSa.id : (vmValues.bootDiagnostics.enabled && (vmValues.bootDiagnostics.newStorageAccount || empty(vmValues.bootDiagnostics.newStorageAccount))) ? newBootDiagsa.outputs.storageAccountId : json('null')
      }
    }
  }
}]
// Calls customScriptExtension module to provision extension on the deloyed vm(s)
module customScriptExtension 'customscriptextension.bicep' = [for e in range(1, vmValues.vmCount): if(!empty(vmValues.customScriptExtension)) {
  name: '${vmValues.vmNamePrefix}-vm${e}-extension'
  params: {
    resourceLocation: resourceLocation
    customScriptExtentionName: '${vmValues.vmNamePrefix}-vm${e}/${vmValues.customScriptExtension.Name}'
...
output resourceId array = [for o in range (1, vmValues.vmCount): {
  resourceId: windowsvm[0].id  
}]
